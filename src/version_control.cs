...
namespace FIA_Biosum_Manager
{
	...
	public class version_control
	{
        ...
        /// <summary>
        /// This was never implemented since the 
        /// VOLTSGRS column does not need to 
        /// get updated at this time.
        /// All previous data is obsolete
        /// because of the condprop fix and 
        /// the user will delete the current
        /// plot data and input again in order 
        /// to get the new condprop values. 
        /// The VOLTSGRS column will update during 
        /// the inputting of new plot records.
        /// </summary>
        private void Update_5_7_0_save()
        {
            frmMain.g_sbpInfo.Text = "Version Update: Updating to version 5.7.0...Stand by";
            ...
            intTreeTableType = oDs.getDataSourceTableNameRow("TREE");
            intCondTableType = oDs.getDataSourceTableNameRow("CONDITION");

            if (intTreeTableType > -1 && intCondTableType > -1)
            {
                //even though listed in datasource table, the file and table could possibly not exist
                if (oDs.m_strDataSource[intTreeTableType, Datasource.FILESTATUS] == "F" &&
                    oDs.m_strDataSource[intTreeTableType, Datasource.TABLESTATUS] == "F")
                {
                    strTreeTable = oDs.m_strDataSource[intTreeTableType, Datasource.TABLE].Trim();
                    strCondTable = oDs.m_strDataSource[intCondTableType, Datasource.TABLE].Trim();
                    strTreeFile = oDs.m_strDataSource[intTreeTableType, Datasource.PATH].Trim() + "\\" +
                                oDs.m_strDataSource[intTreeTableType, Datasource.MDBFILE].Trim();
                    strCondFile = oDs.m_strDataSource[intCondTableType, Datasource.PATH].Trim() + "\\" +
                    ...
                    if (strCondFile.Trim().ToUpper() != strTreeFile.Trim().ToUpper())
                    {
                        oDao.CreateTableLink(strTreeFile, "tempCond", strCondFile, strCondTable);
                        strCondTable = "tempCond";
                    }
                    if (oDao.TableExists(strTreeFile, "fcs_biosum_volume"))
                    {
                        oDao.DeleteTableFromMDB(strTreeFile, "fcs_biosum_volume");
                    }
                    ...
                    for (int z = 1; z <= 5; z++)
                    {
                        ...
                        oDao.m_intError = 0;
                        oDao.CreateOracleXETableLink("FIA Biosum Oracle Services", "FCS", "fcs", "FCS", "BIOSUM_VOLUME", strTreeFile, "fcs_biosum_volume");
                        if (oDao.m_intError == 0) break;
                    }
                    if (oDao.m_intError == 0)
                    {
                        ...
                        //
                        FIADB.Oracle.Services oOracleServices = new FIADB.Oracle.Services();
                        oOracleServices.Start();

                        oOracleServices.m_oTree.GetVolumesMode = FIADB.Oracle.Services.Tree.GetVolumesModeValues.SQLUpdate;
                        if (oAdo.TableExist(oAdo.m_OleDbConnection, Tables.FVS.DefaultOracleInputVolumesTable))
                            oAdo.SqlNonQuery(oAdo.m_OleDbConnection, "DROP TABLE " + Tables.FVS.DefaultOracleInputVolumesTable);
                        frmMain.g_oTables.m_oFvs.CreateOracleInputBiosumVolumesTable(oAdo, oAdo.m_OleDbConnection, Tables.FVS.DefaultOracleInputVolumesTable);

                        if (oAdo.TableExist(oAdo.m_OleDbConnection, Tables.FVS.DefaultOracleInputFCSVolumesTable))
                            oAdo.SqlNonQuery(oAdo.m_OleDbConnection, "DROP TABLE " + Tables.FVS.DefaultOracleInputFCSVolumesTable);
                        frmMain.g_oTables.m_oFvs.CreateOracleInputFCSBiosumVolumesTable(oAdo, oAdo.m_OleDbConnection, Tables.FVS.DefaultOracleInputFCSVolumesTable);
                        ...
                        oAdo.m_strSQL = "INSERT INTO " + Tables.FVS.DefaultOracleInputFCSVolumesTable + " " +
                                     "(" + strColumns + ") SELECT " + strValues + " FROM " + strTreeTable;


                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                            frmMain.g_oUtils.WriteText(frmMain.g_oFrmMain.frmProject.uc_project1.m_strDebugFile, "version_control.Update_5_7_0: " + oAdo.m_strSQL + "\r\n\r\n");
                        oAdo.SqlNonQuery(oAdo.m_OleDbConnection, oAdo.m_strSQL);

                        oAdo.m_strSQL = "UPDATE " + Tables.FVS.DefaultOracleInputFCSVolumesTable + " f INNER JOIN " + strCondTable + " c ON f.CND_CN = c.BIOSUM_COND_ID SET f.vol_loc_grp=IIF(INSTR(1,c.vol_loc_grp,'22') > 0,'S26LEOR',c.vol_loc_grp)";
                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                            frmMain.g_oUtils.WriteText(frmMain.g_oFrmMain.frmProject.uc_project1.m_strDebugFile, "version_control.Update_5_7_0: " + oAdo.m_strSQL + "\r\n\r\n");
                        oAdo.SqlNonQuery(oAdo.m_OleDbConnection, oAdo.m_strSQL);


                        oAdo.m_strSQL = "INSERT INTO fcs_biosum_volume (" + strColumns + ") SELECT " + strColumns + " FROM " + Tables.FVS.DefaultOracleInputFCSVolumesTable;
                        if (frmMain.g_bDebug && frmMain.g_intDebugLevel > 2)
                            frmMain.g_oUtils.WriteText(frmMain.g_oFrmMain.frmProject.uc_project1.m_strDebugFile, "version_control.Update_5_7_0: " + oAdo.m_strSQL + "\r\n\r\n");
                        oAdo.SqlNonQuery(oAdo.m_OleDbConnection, oAdo.m_strSQL);
                        ...
                        oOracleServices.m_oTree.GetBiosumVolumes();




                        if (oOracleServices.m_intError == 0)
                        {
                            oAdo.CloseConnection(oAdo.m_OleDbConnection);
                            oAdo.OpenConnection(oAdo.getMDBConnString(strTreeFile, "", ""));
                            oAdo.m_strSQL = "UPDATE " + strTreeTable + " t " +
                        ...
                        //drop the fcs_biosum_volume table link
                        oAdo.m_strSQL = "DROP TABLE fcs_biosum_volume";
                        oAdo.SqlNonQuery(oAdo.m_OleDbConnection, oAdo.m_strSQL);
                        if (strCondTable == "tempCond")
                        {
                            //drop condition table link
                            oAdo.m_strSQL = "DROP TABLE tempCond";
                            oAdo.SqlNonQuery(oAdo.m_OleDbConnection, oAdo.m_strSQL);
                        }
                        if (oAdo.TableExist(oAdo.m_OleDbConnection, Tables.FVS.DefaultOracleInputVolumesTable))
                            oAdo.SqlNonQuery(oAdo.m_OleDbConnection, "DROP TABLE " + Tables.FVS.DefaultOracleInputVolumesTable);
