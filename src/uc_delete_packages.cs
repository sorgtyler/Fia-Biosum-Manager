...
namespace FIA_Biosum_Manager
{
    public partial class uc_delete_packages : UserControl
    {
        ...
        private void btnFilterFinish_Click(object sender, System.EventArgs e)
        {
            ...
            if (m_intError == 0)
            {
                if (this.rdoFilterByFile.Checked)
                {
                    if (System.IO.File.Exists(this.txtFilterByFile.Text.Trim()) == true)
                    {
                        this.m_strPackageNumbers = this.CreateDelimitedStringList(this.txtFilterByFile.Text.Trim(), ",", ",", false);
                        if (this.m_intError == 0) { DeletePkgsFromBiosumProject_Start(); }
                    }
                    else
                    {
        ...
        private void DeletePkgsFromBiosumProject_Start()
        {
            ...
            this.m_strCurrentProcess = "DeletePackages";
...
//                    .Where(s => s.ToLower().EndsWith(".mdb") || s.ToLower().EndsWith(".accdb")).ToArray());
                ...
                //Look at every access database in every directory TODO: break into chunks for better progress bars
                ConnectToDatabasesInPathAndExecuteDeletes(Directory
                    .GetFiles(m_strProjDir + "\\", "*.*", SearchOption.AllDirectories)
                    .Where(s => s.ToLower().EndsWith(".mdb") || s.ToLower().EndsWith(".accdb")).ToArray());
                ...
                MessageBox.Show(
                    String.Format("Successfully deleted data associated with {0} packages!",
                        m_strPackageNumbers.Split().ToList().Distinct().Count()),
        ...
        {
//            string[] condCNs = m_strPackageNumbers.Split(',');
//
...
//                m_ado.SqlNonQuery(m_connTempMDBFile, "CREATE TABLE Pkgs_to_delete (cn TEXT(34))");
//				m_ado.AddPrimaryKey(m_connTempMDBFile,"Pkgs_to_delete","Pkgs_to_delete_pk","cn");
//                foreach (string condCN in condCNs)
//                {
//                    m_ado.SqlNonQuery(m_connTempMDBFile, "INSERT INTO Pkgs_to_delete (cn) VALUES (" + condCN + ");");
//                }
//
//                m_ado.CreateDataSet(m_connTempMDBFile,
//                    string.Format(
//                        "SELECT c.cn, c.biosum_cond_id, t.cn " +
//                        "FROM (({0} c INNER JOIN Pkgs_to_delete ON c.cn=Pkgs_to_delete.cn) " +
...
//                m_ado.CreateDataSet(m_connTempMDBFile,
//                    String.Format(
//                        "SELECT allPkgs.biosum_plot_id, allPkgs.cn " + //, allconds.cntConds, someconds.cntConds " +
...
//                        m_strCondTable, m_strPlotTable, m_strPackageNumbers), "plots_with_all_conds_deleted");
...
//            };
        ...
        private void ExecuteDeleteOnTables(string strDbPathFile, string[] tables = null, string[] exceptions = null)
        {
            using (var conn = new OleDbConnection(m_ado.getMDBConnString(strDbPathFile, "", "")))
            {
                ...
                foreach (string table in strTables)
                {
                    ...
                    column = null;
                    foreach (string col in new string[] {"rxpackage"}) //TODO: get list of package column names to match or transform to if statement
                    {
        ...
        private int BuildAndExecuteDeleteSQLStmts(OleDbConnection oConn, string table, string column)
        {
            ...
            string[] edgeCaseTables = {"rxpackage"};

            if (edgeCaseTables.Contains(table))
            {
                return 0; //didn't delete anything
            }
...
//            }
            ...
            matchedRecordCount = (int) m_ado.getSingleDoubleValueFromSQLQuery(oConn,
                String.Format("SELECT COUNT(*) FROM {0} WHERE {1} IN ({2});", table, column, m_strPackageNumbers), table);
            //todo: if matchedRecordCount == 0, return early? or check if 0 before executing sql
