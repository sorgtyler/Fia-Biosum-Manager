...
/*
 * ----------PLOT NOTES------------
 * 1. No plot roadless field so defaulted GIS_ROADLESS_YN to "N"
 * 3. No plot accessible field so based value of PLOT_ACCESSIBLE_YN on whether plot was in a
 *		protected area or not
 * 4. No number of conditions on a plot field so defaulted NUM_COND to "1"
 * 5. Default ONE_COND_YN value is "Y"
 * 6. No plot measurement date so NULL values for MEASYEAR,MEASMON,MEASDAY fields
 * 7. IDB_PLOT_ID is used to contain the record number.
 * 8. There is a STATE, COUNTY value but no PLOT value.  Saved
 *    the IDB_CONDITION value to the PLOT field.
 * 9. No plot unit code, cyle and subcycle values so defaulted 
 *    UNITCD value to NULL
 * 10. No elevation, fvs variant, half state, subplot count values so
 *     default ELEVFT, FVS_VARIANT,HALF_STATE,and SUBPLOT_COUNT_PLOT fields
 *     have a value of NULL.
 */
namespace FIA_Biosum_AZNM_Data_Converter
{
	...
	public class convert_data
	{
		...
		public convert_data()
		{
		    ...
		    this.m_txtFileStream = new FileStream("c:\\fia_biosum\\aznm_converted_data\\convert_data_log.txt", FileMode.Create, 
		                                 FileAccess.Write);
			this.m_txtStreamWriter = new StreamWriter(this.m_txtFileStream);
			this.m_txtStreamWriter.Write("Convert Arizona And New Mexico Data " + System.DateTime.Now.ToString() + "\n");
	  ...
	  ~convert_data()
		...
		public void convert_all()
		{
			...
			m_frmTherm.Show();
			m_frmTherm.Focus();
			m_frmTherm.Text = "Converting Arizona And New Mexico Previous Study Data";
			m_frmTherm.Refresh();
...
//			string strIdbMDBFile = "c:\\fia_biosum\\orca_converted_data\\db\\idb version 2.mdb";
	    string strSourceMDBFile = "c:\\fia_biosum\\aznm_converted_data\\db\\aznm_core_saf.mdb";
	    string strTargetMDBFile = "c:\\fia_biosum\\aznm_converted_data\\db\\master.mdb";
			string strScenarioMDBFile = "c:\\fia_biosum\\aznm_converted_data\\core\\db\\scenario.mdb";

			string strTargetTravelTimesMDBFile = "c:\\fia_biosum\\aznm_converted_data\\gis\\db\\gis_travel_times.mdb";
			this.m_txtStreamWriter.Write ("Using These MDB Files:\n");

			this.m_txtStreamWriter.Write ("Previous Study Source:" + strSourceMDBFile + " \n Target Source:" + strTargetMDBFile); 
      ...
      string strelevft, strfvsvariant,strhalfstate,strsubplotcountplot;
			...
			try
			{
				...
				strSQL = "DELETE * FROM TARGET_TRAVEL_TIMES_PLOT";
				this.m_ado.SqlNonQuery(this.m_TempMDBFileConn, strSQL);
			...
			if (this.m_intError==0)
			{
				for (intCurrRec = 0; intCurrRec <= p_ds.Tables["source_plot"].Rows.Count-1;intCurrRec++)
				{
					...
					strplotid = Convert.ToString(intCurrRec + 1); //Convert.ToString(p_ds.Tables["source_plot"].Rows[intCurrRec]["idb_condition"]); //     this.m_ado.m_OleDbDataReader["idb_plot"]);
					strstatecd = Convert.ToString(p_ds.Tables["source_plot"].Rows[intCurrRec]["state"]); //this.m_ado.m_OleDbDataReader["state"]);
					strcountycd = Convert.ToString(p_ds.Tables["source_plot"].Rows[intCurrRec]["county"]); //this.m_ado.m_OleDbDataReader["cty"]);
					strplot = Convert.ToString(p_ds.Tables["source_plot"].Rows[intCurrRec]["idb_condition"]); //Convert.ToString(this.m_ado.m_OleDbDataReader["plot"]);
					...
					strmovedfromprotected = Convert.ToString(p_ds.Tables["source_plot"].Rows[intCurrRec]["in_out"]); //Convert.ToString(this.m_ado.m_OleDbDataReader["in_out_code"]);
					if (strmovedfromprotected.IndexOf("T",0) < 0)
					{
					...
					strDbTravelTimesFields = "INSERT INTO target_travel_times_plot (biosum_plot_id,idb_plot_id";
					strDbTravelTimesValues="VALUES ('" + strbiosumid + "'," + strplotid ;
		...
		private void convert_cond()
		{
			...
			if (this.m_intError==0)
			{
				for (intCurrRec = 0; intCurrRec <= p_ds.Tables["source_cond"].Rows.Count-1;intCurrRec++)
				{
					...
					strDbFields = "INSERT INTO target_cond (biosum_cond_id,idb_cond_id,biosum_plot_id";
					strValues=" VALUES ('" + strbiosumcondid + "'," + strcondid + ",'" + strbiosumid + "'" ;
		...
		private void convert_ffe()
		{
			...
			if (this.m_intError==0)
			{
				for (intCurrRec = 0; intCurrRec <= p_ds.Tables["source_ffe"].Rows.Count-1;intCurrRec++)
				{
					...
					//strpostmortbasev = Convert.ToString(p_ds.Tables["source_ffe"].Rows[intCurrRec]["sev_cf_mort"]); 
					//strpostmortbamod = Convert.ToString(p_ds.Tables["source_ffe"].Rows[intCurrRec]["mod_cf_mort"]); 
