...
namespace FIA_Biosum_Manager
{
	...
	/// </summary>
	public class uc_sql_builder_new : System.Windows.Forms.UserControl
	{
		private env m_oEnv;
		...
		public string m_strTempA_FILENAME;
		public System.Data.OleDb.OleDbConnection m_TempA_FILENAMEConn;
		...
		public uc_sql_builder_new(System.Windows.Forms.ListView p_lvDataSource)
		{
			this.lvDataSource = p_lvDataSource;
...
//				}
		...
		private void btnTest_Click(object sender, System.EventArgs e)
		{
			...
			strConn = p_ado.getMDBConnString(this.m_strTempA_FILENAME ,"","");
		...
		private void btnGetTables_Click(object sender, System.EventArgs e)
		{
			...
			MessageBox.Show("btnGetTables_Click function not ready");
			return;
		...
		private void lstTables_SelectedValueChanged(object sender, System.EventArgs e)
		{
			...
			MessageBox.Show("lstTables_SelectedValueChanged function not ready");
			return;
		...
		private void lstFields_SelectedValueChanged(object sender, System.EventArgs e)
		{
			...
			MessageBox.Show("lstFields_SelectedValueChanged function not ready");
			return;
		}
		...
		private void btnOK_Click(object sender, System.EventArgs e)
		{
			...
			MessageBox.Show("btnOK_Click not ready");
			return;
		...
		{
			MessageBox.Show("btnPrevSql not ready");
		...
		{
			MessageBox.Show("btnExecute_Click not ready");
			return;
		}
		public void InitializeDataSource(System.Windows.Forms.ListView lv)
		{
			 ...
			 *************************/
			this.m_strTempA_FILENAME =  p_utils.getRandomFile(this.m_oEnv.strTempDir,"mdb");
			p_utils = null;
			 ...
			 *****************************************************/
			p_dao.CreateMDB(this.m_strTempA_FILENAME);
				...
				if (lLoaded==false)
				{
					for (x=0; x<=this.lvDataSource.Items.Count-1;x++)
					{
						...
						if (this.lstTables.Items[y].ToString().Trim().ToUpper() 
							==
							this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLE].Text.Trim().ToUpper())
						{
							strFullPathMDB = this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.PATH].Text.Trim() + 
								"\\" + this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.MDBFILE].Text.Trim();
							
							//used to create a link to the table
							dao_data_access p_dao = new dao_data_access();

							//create a link to the table in an mdb file
							p_dao.CreateTableLink(this.m_strTempA_FILENAME,this.lstTables.Items[y].ToString().Trim(),
								strFullPathMDB,this.lstTables.Items[y].ToString().Trim());
							p_dao = null;

							//connect to mdb file that will be used as the master table link file
							this.m_TempA_FILENAMEConn = new System.Data.OleDb.OleDbConnection();
							strConn = p_ado.getMDBConnString(this.m_strTempA_FILENAME,"","");
							p_ado.OpenConnection(strConn, ref this.m_TempA_FILENAMEConn);
							...
							strSQL = "SELECT * FROM " + this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLE].Text.Trim();
							this.m_OleDbDataAdapter.SelectCommand = new System.Data.OleDb.OleDbCommand(strSQL,this.m_TempA_FILENAMEConn);
							try 
							{

								this.m_OleDbDataAdapter.Fill(this.m_dsDataSource,this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLE].Text.Trim());
							}
							catch (Exception e)
							{
							...
							this.m_strListedTablesLoadedIntoDatasets[this.m_intNumberOfTablesLoadedIntoDatasets] = this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLE].Text.Trim();
							this.m_intNumberOfTablesLoadedIntoDatasets++;

							this.m_TempA_FILENAMEConn.Close();
							this.m_TempA_FILENAMEConn= null;
		...
		public void getNumberOfValidTables()
		{
			...
			for (x=0; x <= this.lvDataSource.Items.Count - 1; x++)
			{
				if (this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLESTATUS].Text.Trim().ToUpper()=="FOUND" &&
					this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.FILESTATUS].Text.Trim().ToUpper()=="FOUND")
				{
		...
		public void AddDataSourceTablesIntoListBox(System.Windows.Forms.ListBox p_lst)
		{
			...
			for (x=0; x <= this.lvDataSource.Items.Count - 1; x++)
			{
				if (this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLESTATUS].Text.Trim().ToUpper()=="FOUND" &&
					this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.FILESTATUS].Text.Trim().ToUpper()=="FOUND")
				{
					p_lst.Items.Add(this.lvDataSource.Items[x].SubItems[uc_sql_builder_new.TABLE].Text.Trim());
				}
			}
		}
		...
		{
			MessageBox.Show("getDataSourceTableNameRow Not Ready");
			return -1;
