...
namespace FIA_Biosum_Manager
{
	...
	public class RxTools
	{
        ...
        /// <summary>
        /// Load all the Harvest Methods and their properties into a variable collection
        /// </summary>
        /// <param name="p_oAdo"></param>
        /// <param name="p_oConn"></param>
        /// <param name="p_oQueries"></param>
        /// <param name="p_oFRCSHarvestMethodItem_Collection"></param>
        public void LoadHarvestMethods(ado_data_access p_oAdo,System.Data.OleDb.OleDbConnection p_oConn,Queries p_oQueries, uc_processor_scenario_run.FRCSHarvestMethodItem_Collection p_oFRCSHarvestMethodItem_Collection)
		{
            int x;
            if (p_oFRCSHarvestMethodItem_Collection==null)
                p_oFRCSHarvestMethodItem_Collection = new uc_processor_scenario_run.FRCSHarvestMethodItem_Collection();

            if (p_oFRCSHarvestMethodItem_Collection.Count > 0)
            {
                for (x=p_oFRCSHarvestMethodItem_Collection.Count-1;x>=0;x--)
                {
                    p_oFRCSHarvestMethodItem_Collection.Remove(x);
                }
            }
			p_oAdo.m_strSQL = Queries.GenericSelectSQL(p_oQueries.m_oReference.m_strRefHarvestMethodTable,"*","steep_yn IN ('Y','N')");
			p_oAdo.SqlQueryReader(p_oAdo.m_OleDbConnection,p_oAdo.m_strSQL);
			...
			if (p_oAdo.m_intError==0)
			{
				if (p_oAdo.m_OleDbDataReader.HasRows)
				{
					while (p_oAdo.m_OleDbDataReader.Read())
					{
						if (p_oAdo.m_OleDbDataReader["method"] != System.DBNull.Value)
						{
                            uc_processor_scenario_run.FRCSHarvestMethodItem oItem = new uc_processor_scenario_run.FRCSHarvestMethodItem();
                            //harvest method
                            oItem.HarvestMethod = Convert.ToString(p_oAdo.m_OleDbDataReader["method"]).Trim();
                            //desc
                            if (p_oAdo.m_OleDbDataReader["description"] != System.DBNull.Value)
                            {
                                oItem.Description = Convert.ToString(p_oAdo.m_OleDbDataReader["description"]).Trim();
                            }
                            //steep
							if (p_oAdo.m_OleDbDataReader["steep_yn"].ToString().Trim()=="Y")
							{
								 oItem.SteepSlope=true;
							}
							else
							{
								 oItem.SteepSlope = false;
							}
                            //chips
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3Chips"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeChips = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3Chips"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeChips=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3ChipsDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeChipsDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3ChipsDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeChipsDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3ChipsCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeChipsCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3ChipsCellLocation"]).Trim();
                            }
                            //small logs
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogs"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeSmLogs = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogs"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeSmLogs=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogsDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeSmLogsDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogsDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeSmLogsDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogsCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeSmLogsCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3SmallLogsCellLocation"]).Trim();
                            }
                            //large logs
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogs"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogs = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogs"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeLgLogs=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogsDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogsDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogsDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeLgLogsDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogsCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogsCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LargeLogsCellLocation"]).Trim();
                            }
                            //all logs
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogs"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllLogs = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogs"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeAllLogs=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogsDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllLogsDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogsDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeAllLogsDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogsCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllLogsCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllLogsCellLocation"]).Trim();
                            }
                            //all trees
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTrees"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllTrees = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTrees"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeAllTrees=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTreesDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllTreesDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTreesDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeAllTreesDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTreesCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeAllTreesCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3AllTreesCellLocation"]).Trim();
                            }
                            //percent of large logs to all logs
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogs"] != System.DBNull.Value)
                            {
                                oItem.MaxLgLogsToAllLogsPercent = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogs"]);
                            }
                            else
                            {
                                oItem.MaxLgLogsToAllLogsPercent = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogsDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxLgLogsToAllLogsPercentDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogsDefault"]);
                            }
                            else
                            {
                                oItem.MaxLgLogsToAllLogsPercentDefault = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogsCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxLgLogsToAllLogsPercentCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3PercentLgLogsToAllLogsCellLocation"]).Trim();
                            }
                            //large logs to per acre
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcre"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogsPerAcre = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcre"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeLgLogsPerAcre = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcreDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogsPerAcreDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcreDefault"]);
                            }
                            else
                            {
                                oItem.MaxCubicFootVolumeLgLogsPerAcreDefault = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcreCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxCubicFootVolumeLgLogsPerAcreCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxTreeVolumeFt3LgLogsPerAcreCellLocation"]).Trim();
                            }
                            //yarding distance
                            if (p_oAdo.m_OleDbDataReader["MaxYardingDistance"] != System.DBNull.Value)
                            {
                                oItem.MaxYardingDistance = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxYardingDistance"]);
                            }
                            else
                            {
                                oItem.MaxYardingDistance=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxYardingDistanceDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxYardingDistanceDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxYardingDistanceDefault"]);
                            }
                            else
                            {
                                oItem.MaxYardingDistanceDefault=-1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxYardingDistanceCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxYardingDistanceCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxYardingDistanceCellLocation"]).Trim();
                            }
                            //slope percent
                            if (p_oAdo.m_OleDbDataReader["MaxSlopePercent"] != System.DBNull.Value)
                            {
                                oItem.MaxSlopePercent = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxSlopePercent"]);
                            }
                            else
                            {
                                oItem.MaxSlopePercent = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxSlopePercentDefault"] != System.DBNull.Value)
                            {
                                oItem.MaxSlopePercentDefault = Convert.ToInt32(p_oAdo.m_OleDbDataReader["MaxSlopePercentDefault"]);
                            }
                            else
                            {
                                oItem.MaxSlopePercentDefault = -1;
                            }
                            if (p_oAdo.m_OleDbDataReader["MaxSlopePercentCellLocation"] != System.DBNull.Value)
                            {
                                oItem.MaxSlopePercentCellLocation = Convert.ToString(p_oAdo.m_OleDbDataReader["MaxSlopePercentCellLocation"]).Trim();
                            }
                            oItem.InCurrentBatch = false;
                            p_oFRCSHarvestMethodItem_Collection.Add(oItem);
